...
class WaveHunterV9Policy(ActorCriticPolicy):
    ...
    def compute_aux_loss(self, obs, a1_bins, a2_label, valid_mask, peak_label=None, b1_label=None):
        ...
        if peak_valid.any():
            ...
            # Focal Loss: 期权 Gamma 思想，聚焦稀有难样本 (Peak 2.56%)
            p = torch.sigmoid(logit_v)
            pt = torch.where(target_v > 0.5, p, 1 - p)
            alpha = torch.where(target_v > 0.5, torch.tensor(0.75, device=pf.device), torch.tensor(0.25, device=pf.device))
            gamma = 2.0
            bce = nn.functional.binary_cross_entropy_with_logits(logit_v, target_v, pos_weight=pos_weight, reduction='none')
            focal_weight = alpha * (1 - pt).pow(gamma)
            peak_loss = (focal_weight * bce).mean()
            pred_v = p > 0.5
            true_v = target_v > 0.5
